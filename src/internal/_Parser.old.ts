...
import { Position } from './_StringStream.old';
import {
  BACKTICK,
  DIGIT_REGEX,
  DOUBLE_QUOTE,
...
import { TokenStream } from './_TokenStream.old';
import { TokenIs } from './_Token.old';
  ...
  function normalizeChildren(nodes: Array<Children>): Array<Children> {
    return (
      nodes
        ...
        // convert whitespace only text to Whitespace node
        .map((item) => {
          if (NodeIs.Text(item)) {
  ...
  function parseNextChildren(currentComponent: CurrentComponent): Children | 'CLOSE' {
    ...
    if (currentComponent.type === 'RAW') {
      ...
      if (tag === null) {
        ...
        return parseTextChildren({ mode: 'raw', skipFirst: !TokenIs.Text(input.peek()) });
      }
      // in RawElement
      if (peek('#>')) {
        skip('#>');

...
import { Ast, parseDocument, serialize, Utils } from "../mod.ts";

Deno.test("should filter item", () => {
  ...
  const withoutComponent = Utils.filter(
    parsed.result,
    (node) => Ast.NodeIs.SelfClosingElement(node) === false,
  );
  expect(serialize(withoutComponent)).toBe("Hello  Foo <|Content>Hello  </>");
});

Deno.test("should clone at paths", () => {
  const obj = {
    foo: [1, 2, 3],
    bar: {
...
Deno.test("should transform item", () => {
  const doc = `Hello </Component/> Foo </Bar/><|Content>Hello </Bold/> </>`;
  const parsed = parseDocument(doc, "source.docsy");
  ...
  expect(serialize(updated)).toEqual(
    `Hello </Component/> Foo </Bar/><|Content>Hello </Italic/> </>`,
  );
});

Deno.test("should correctly transform array and object to node", () => {
...
// Deno.test('should component name and props', () => {
...
// Deno.test('should override sub changes', () => {
//   const doc = `<|SomeComponent foo="bar" num=41>Inner content|>`;
//   const parsed = parseDocument(doc, 'source.docsy');
//   const updated = Utils.transform(parsed.document, (node) => {
//     if (NodeIs.Num(node)) {

...
import { DocsyErreur, parseDocument, parseExpression } from "../mod.ts";
...
Deno.test(`Parse Empty ExpressionDocument`, () => {
  const file = "";
...
Deno.test(`Parse ExpressionDocument`, () => {
  const file = "42";
...
Deno.test(`Parse ExpressionDocument with whitespace`, () => {
  const file = "   42   ";
  expect(() => parseExpression(file, "expression.docsy")).not.toThrow();
  const result = parseExpression(file, "expression.docsy").result as any;
  expect(result.kind).toBe("ExpressionDocument");
...
Deno.test(`Parse ExpressionDocument with comment`, () => {
  const file = "// some comment\n42";
  expect(() => parseExpression(file, "expression-comments.docsy")).not
    .toThrow();
  const result = parseExpression(file, "expression-comments.docsy")
    .result as any;
  expect(result.kind).toBe("ExpressionDocument");
  expect(result.value.kind).toBe("Num");
  expect(result.value.value).toBe(42);
...
Deno.test(`Parse comment at end of file`, () => {
  const file = "// some comment";
  expect(() => parseExpression(file, "expression.docsy")).not.toThrow();
  const result = parseExpression(file, "expression.docsy").result as any;
  expect(result.kind).toBe("ExpressionDocument");
...
Deno.test(`Parse ExpressionDocument object`, () => {
...
Deno.test(`Parse ExpressionDocument with many before and after`, () => {
  const file = "// some comment\n/* Block comment */42  \n  ";
  expect(() => parseExpression(file, "expression.docsy")).not.toThrow();
  const result = parseExpression(file, "expression.docsy").result as any;
  expect(result.kind).toBe("ExpressionDocument");
...
Deno.test(`Parse a document with text`, () => {
  const file = `Hello`;
...
Deno.test(`Parse a document with whitespaces`, () => {
  const file = `\n \n `;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse a document with text and whitespaces`, () => {
  const file = `\n \n Hello`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse a document with multiple text and whitespaces`, () => {
  const file = `\n \n Hello Foo \n Bar`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse SelfClosingElement`, () => {
  const file = `</Demo/>`;
...
Deno.test(`Parse SelfClosingElement then whitespace`, () => {
  const file = `</Demo/> `;
  expect(() => parseDocument(file, "self-closing-whitespace.docsy")).not
    .toThrow();
  const result = parseDocument(file, "self-closing-whitespace.docsy")
    .result as any;
...
Deno.test(`Parse Element unamed close`, () => {
...
Deno.test(`Parse single whitespaces between two tag`, () => {
  const file = `</Demo/> </Demo/>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
...
Deno.test(`Parse double whitespaces`, () => {
  const file = `</Demo/>\n\n`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(2);
...
Deno.test(`Parse Line element eof`, () => {
  const file = `<Demo> Hello`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse Line element with elem inside`, () => {
...
Deno.test(`Parse single whitespaces`, () => {
  const file = `</Demo/>\n`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(2);
...
Deno.test(`Parse element name with ElementNameMember`, () => {
  const file = `</Demo.Foo/>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse SelfClosingElement with attributes`, () => {
...
Deno.test(`Parse SelfClosingElement with attributes`, () => {
  const file =
    `</Title bold foo="bar" demo=-3.14 admin=true mode=null onClick=undefined />`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
  expect(result.children[0].kind).toEqual("SelfClosingElement");
...
Deno.test(`Parse backtick attributes`, () => {
...
Deno.test(`Parse empty array attribute`, () => {
...
Deno.test(`Parse array attribute`, () => {
...
Deno.test(`Parse array of array`, () => {
...
Deno.test(`Parse object attribute`, () => {
...
Deno.test(`Parse object with quoted key`, () => {
...
Deno.test(`Parse open/close tag`, () => {
...
Deno.test(`Parse named close tag`, () => {
...
Deno.test(`Parse simple element with no content`, () => {
  const file = `<|Demo><Demo/>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
});

Deno.test(`Throw when you close the wrong tag`, () => {
  ...
  expect(DocsyErreur.get(error)).toEqual({
    docsyStack: [
      {
        message: "Invalid close tag: wrong component",
        name: "Element",
        position: 21,
      },
      {
        message: "Parser at index 1 did not match",
        name: "Document.1",
        position: 0,
      },
    ],
...
Deno.test(`Throw when you invalid tag`, () => {
  ...
  expect(DocsyErreur.get(error)).toEqual({
    docsyStack: [
      {
        message: "EOF reached",
        name: "LineElement.4.Exact('>')",
        position: 21,
      },
      {
        message: "Parser at index 4 did not match",
        name: "LineElement.4",
        position: 21,
      },
      {
        message: "Parser at index 6 did not match",
        name: "Element.6",
        position: 16,
      },
      {
...
Deno.test(`Parse dot member`, () => {
  const file = `</Demo foo=Utils.size />`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
});

Deno.test(`Parse dot function call`, () => {
...
Deno.test(`Parse props with object`, () => {
...
Deno.test(`Parse a line comment`, () => {
  const file = `// some comment`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
...
Deno.test(`Parse an empty line comment`, () => {
  const file = `//\n`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children[0].kind).toBe("LineComment");
...
Deno.test(`Parse an empty line comment then EOF`, () => {
  const file = `//`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children[0].kind).toBe("LineComment");
...
Deno.test(`Parse block comment ended with EOF`, () => {
  const file = `/* some comment \n\n`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children[0].kind).toBe("BlockComment");
...
Deno.test(`Ending < does not throw error`, () => {
  const file = `</Foo/><`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
...
Deno.test(`Parse a line in context`, () => {
...
Deno.test(`Does not parse element in comment`, () => {
  const file = `// </Foo/>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children[0].kind).toBe("LineComment");
...
Deno.test(`Fragment with content`, () => {
  ...
  expect(children.map((v: any) => v.kind)).toEqual([
    "Text",
    "SelfClosingElement",
    "Text",
  ]);
...
Deno.test(`Two Fragment side by side`, () => {
  const file = `<|>Foo</><|>Bar</>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(2);
...
Deno.test(`Fragment in Fragment`, () => {
  const file = `<|>A1<|>B1</>A2</>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Fragment > Element > Fragment`, () => {
...
Deno.test("Element in Raw Fragment is text", () => {
  const file = `<#> </Demo/> </>`;
...
Deno.test("Parse two Raw fragments", () => {
...
Deno.test("Parse two Raw fragments with new lines", () => {
  const file = `<#>\n\nSome raw content</>\n<#>Som raw content</>`;
  expect(() => parseDocument(file, "source.docsy").result).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(3);
...
Deno.test("RawElement name close", () => {
...
Deno.test("Element in RawElement is text", () => {
  const file = `<#Foo> </Hello/> <Foo/>`;
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.children.length).toBe(1);
...
Deno.test(`Parse raw.docsy file`, () => {
...
Deno.test(`Parse complex-raw.docsy file`, () => {
...
Deno.test(`Parse complete.docsy file`, () => {
  const file = readFile("complete");
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
});

Deno.test(`Parse inject`, () => {
  const file = "{34}";
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.kind).toBe("Document");
...
Deno.test(`Parse inject with spaces`, () => {
  const file = "{\n34   \n}";
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
  const result = parseDocument(file, "source.docsy").result as any;
  expect(result.kind).toBe("Document");
...
Deno.test(`Parse all.docsy file`, () => {
  const file = readFile("all");
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
});

Deno.test(`Parse long.docsy file`, () => {
  const file = readFile("long");
  expect(() => parseDocument(file, "source.docsy")).not.toThrow();
});

Deno.test(`Parse code with JSX inside`, () => {

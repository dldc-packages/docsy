...
test(`Parse a document with text`, () => {
  ...
  const result = DocsyParser.parseDocumentSync(file).document as any;
...
test(`Throw when you close the wrong tag`, () => {
  ...
  expect(() => DocsyParser.parseDocumentSync(file)).toThrow(DocsyParsingError);
});
...
test(`Ending < throw error`, () => {
  const file = `<|Foo|><`;
  expect(() => DocsyParser.parseDocumentSync(file)).toThrow();
});
...
test('Element in Raw Fragment is text', () => {
  const file = `<#> <|Demo|> <#>`;
  expect(() => DocsyParser.parseDocumentSync(file).document).not.toThrow();
  const result = DocsyParser.parseDocumentSync(file).document as any;
  expect(result.nodes.children.length).toBe(1);
  expect(result.nodes.children[0].type).toEqual('RawFragment');
  expect(result.nodes.children[0].nodes.children.length).toEqual(1);
...
test(`Parse ExpressionDocument`, () => {
  const file = '42';
  expect(() => DocsyParser.parseExpressionSync(file)).not.toThrow();
  const result = DocsyParser.parseExpressionSync(file).expression as any;
...
test(`Parse long.docsy file asyn`, async () => {
  const file = readFile('long');
  await expect(DocsyParser.parseDocument(file)).resolves.not.toThrow();
}, 20000);
